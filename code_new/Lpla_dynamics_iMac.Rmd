...
tpm_i_long$Taxon <- factor(tpm_i_long$Taxon, levels = c("S103","S239","B89"))
tpm_pools_long$Taxon <- factor(tpm_pools_long$Taxon, levels = c("S103","S239","B89"))
...
ggplot(tpm_i_long, aes(x = generation, y = TPM, fill = Taxon)) +
    geom_area() +
    ylab("Taxon relative Abundance") +
    scale_fill_manual(values = c("S239" = "red", "S103" = "lightblue", "B89" = "grey")) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
