...
tree_path <- normalizePath(paste0(cov_path,"/tree/Lpla.phylip.treefile"))
...
genmap <- gggenomes(seqs= fa,feats = gff) %>% 
                          add_feats(island)
...
# Read the tree and re-root it to the midpoint
...
# tree_plot + geom_text(aes(label = node), hjust = -0.3) # This gives you the node numbers of the tree, in order to manipulate them
tree_plot <- flip(tree_plot, 7, 8) # nodes 7 and 8 correspond to B246 and LpWF. We flip the branch for aesthetic reasons, so that LpWF is at the bottom of the tree.
...
meta_LpWF <- data.frame(sample = "LpWF",
                        Temperature = "NA",
                        Generation = "NA",
                        Population = "NA",
                        Replicate = "NA")
...
# Import the coverage features of the plasmid in each genome
cov_feats <- fread(paste0(cov_path,"/coverage.tsv"), header=T) %>% 
  ...
  mutate(colour = case_when(
    ...
    sample %in% "LpWF" ~ "darkgrey",
    Temperature == "HOT" ~ "red",
    Temperature == "BASE" ~ "grey",
    Temperature == "COLD" ~ "lightblue",
    Temperature == "COLD-CONSTANT" ~ "lightblue")) %>% 
  mutate(sample = factor(sample, levels = genome_order)) %>%
  arrange(sample) %>% 
  filter(!is.na(sample))

# Plot the coverage of the plasmid in each sample
cov_plot <- ggplot(cov_feats) + 
  geom_tile(aes(start, sample, fill=colour)) + 
  scale_fill_identity() + 
  scale_colour_identity() +
  theme_minimal() +
  theme_void() +
  ...
  geom_text(aes(x = min(start) - 1000, y = sample, label = sample, color = colour), hjust = 1, size = 5)

# Plot the combined figure
combined_plot <- cov_plot %>% insert_left(tree_plot, width=0.1) %>% insert_bottom(genmap_plot, height=.1)

ggsave(filename = "map_isolates_all.png", plot = combined_plot, path = visuals_path, width = 30, height = 12, dpi = 300)


#######################################################################
# DRAW THE GENETIC MAP OF THE PLASMID AND THE COVERAGE PER GENERATION #
#######################################################################
...
# Plot the combined figure
combined_pools <- pools_plot %>% insert_bottom(genmap_plot, height=.1)

ggsave(filename = "map_pools.png", plot = combined_pools, path = visuals_path, width = 30, height = 12, dpi = 300)
...
# Read the tree, re-root it to the midpoint, plot it and save the order of the genomes
tree <- read.tree(tree_path)
tree <- ladderize(tree, right = FALSE)
tree <- midpoint.root(tree)
...
ggsave(filename = "tree.png", plot = tree_plot, path = visuals_path, width = 10, height = 20, dpi = 300)
