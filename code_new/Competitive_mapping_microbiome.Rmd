...
coverage <- fread(reads_path, header=T)
...
```{r Reads normalisation to rpkm}

# Calculate total reads per sample
total_reads <- as.data.frame(colSums(coverage))

# Calculate RPKM
coverage_rpkm <- coverage %>%
  mutate(across(everything(), ~ . / size)) %>%
  mutate(across(everything(), ~ . / (total_reads / 1e6)))
